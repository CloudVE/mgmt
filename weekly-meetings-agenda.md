...
- CloudMan crash loops waiting for keycloak to be ready. Having an init-container that waits for keycloak will reduce this delay.
...
## 2020-09-15
- Issues to resolve:
  - a. Set password to ssh key generated by cloudlaunch
  - b. K8S jobs should be modified to map in only the input files directory in read-only mode, as opposed to entire PVC
